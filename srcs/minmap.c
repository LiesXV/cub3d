...
/*   Updated: 2023/10/04 16:40:46 by ibenhaim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_ray(t_cube *cube)			//init structures and values
{
	...
	cube->r->a = cube->player->a;	// ra ---> cube->r->a (r : ray) (a : angle)
	if (cube->r->a < 0)
		cube->r->a += 2 * PI;
	if (cube->r->a > 2 * PI)
		cube->r->a -= 2 * PI;
...
void	draw_rays(t_cube *cube)
{
	...
	while (r < 30)
	{
		...
		if (cube->r->v->dis<cube->r->h->dis)
		{
			hit.x = cube->r->v->x;
			if (cube->r->a > P2 && cube->r->a < P3)
				hit.y = cube->r->v->y + cube->bloc_size;
			else
				hit.y = cube->r->v->y;	
			tdis = cube->r->v->dis;
		}
